...
def main():
    ...
    parser.add_argument(
        "--target", required=True, help="URL for http mode or 'inproc' for inproc mode"
    )
    ...
    for i, s in enumerate(
        stream_dataset_samples(num_samples=args.num_samples, split=args.dataset_split)
    ):
    ...
    if args.mode == "http":
        ...
        def inproc_fn(sample_bytes):
            ...
            raise NotImplementedError(
                "Inproc mode requires a callable. Use custom script to call process_sample directly."
            )

        print(
            "Note: inproc mode not implemented in CLI wrapper. Use the concurrency_runner.run_concurrency_test() from Python directly with a callable."
        )
